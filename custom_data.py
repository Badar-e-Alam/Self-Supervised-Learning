...
class Image_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # )
        x = PIL.Image.open(img_loc1).convert("RGB").resize((210, 210))
        # image2 = Image.open(img_loc2).resize((200, 200))
        image1 = self.transform(x)
...
class Winding_Dataset(Dataset):
    def __init__(self, csv_file, root_dir):
        ...
        columns_to_drop = ["Unnamed: 0","binary_NOK","CoilNumber"]
    ...
    def __getitem__(self, idx):
        with warnings.catch_warnings():
            ...
            column_names = self.data.columns[
                1:
            ]  # Get the names of all columns starting from the third one
